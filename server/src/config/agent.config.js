...
const ApplicationSchema = z.object({
    ...
    dependencies: z.record(z.string(), z.string()).optional().describe("NPM dependencies with version")
})
...
async function creatApplicationFiles(baseDir, folderName, files) {
    ...
    for (const file of files){
        ...
        printSystem(chalk.green(`  ✅ ${filePath}`))
...
export async function generateApplication(description, aiService, cwd = process.cwd()) {
    try {
        ...
        printSystem(chalk.green.bold('\n✨ Application created successsfully!\n'))
        printSystem(chalk.cyan(`📁 Locatation: ${chalk.bold(appDir)}\n`))
        ...
        printSystem(chalk.red(`\n❌ Error genrationg application: ${error.message}\n`))
        if(error.stack){
